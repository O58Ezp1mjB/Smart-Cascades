...
from pytorch_tabnet.TSP_implement import create_dataloaders, train_batch, TSP_TabNet
...
class TabModel(BaseEstimator):
    ...
    def fit(
        ...
        # For TSP
        self.input_dim = int(X_train.shape[1]/2)
        ...
        for epoch_idx in range(self.max_epochs):
            ...
            for eval_name, valid_dataloader in zip(eval_names, valid_dataloaders):
                self._TSP_predict_epoch(eval_name, valid_dataloader)
    ...
    def _train_epoch(self, train_loader):
        ...
        for batch_idx, (X, y) in enumerate(train_loader):
            ...
            # train_batch from TSP
            batch_logs = train_batch(X, y, self.network, self.loss_fn, self.device, self.lambda_sparse, self.clip_value, self._optimizer)
    ...
    def _TSP_predict_epoch(self, name, loader):
        """
        ...
        for batch_idx, (X, y) in enumerate(loader):
            ...
            scores_shallow = self._predict_batch(X[0])
            scores_deep = self._predict_batch(X[1])

            list_y_true.append(y)
            list_score_shallow.append(scores_shallow)
            list_score_deep.append(scores_deep)
            
            stack_X[0].append(X[0])
            stack_X[1].append(X[1])
        
        stack_X=[torch.cat(stack_X[0],dim=0),torch.cat(stack_X[1],dim=0)]
        y_true = np.vstack(list_y_true)
        score_shallow = np.vstack(list_score_shallow)
        score_deep = np.vstack(list_score_deep)

        metrics_logs = self._metric_container_dict[name]([score_shallow, score_deep, stack_X], y_true)
    ...
    def _set_network(self):
        ...
        torch.manual_seed(self.seed)
        self.network = TSP_TabNet(  # For TSP
            self.input_dim,
            self.output_dim,
            n_d=self.n_d,
            n_a=self.n_a,
